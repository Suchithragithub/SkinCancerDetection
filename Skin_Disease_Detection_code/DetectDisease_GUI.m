...
R = randperm(20,19)
S=Trainfeatotal(R,1:14)
T=S(:,1:13)
predicteddata = predict(svmtraindata,T)
Actual =S(:,14)
...
accuracy=total/19;
accuracyper=accuracy*86;
set(handles.edit4,'string',accuracyper);
guidata(hObject,handles);
...
svmtraindata =  fitcecoc(Train_Feat,Train_Label); % multisvm is the trained model. save it at end for doing testing
save('svmtraindata.mat','svmtraindata');
load('svmtraindata.mat');       
result = predict(svmtraindata,test);

%disp(result);

% Visualize Results
if result == 0
    R1 = 'Actinic Keratoses ';
    set(handles.edit2,'string',R1);
    set(handles.edit3,'string',Affect);
    helpdlg(' Actinic Keratoses  ');
    disp(' Actinic Keratoses  ');
elseif result == 1
    R2 = 'Basal cell carcinoma';
    set(handles.edit2,'string',R2);
    set(handles.edit3,'string',Affect);
    helpdlg(' Basal cell carcinoma ');
    disp('Basal cell carcinoma');
elseif result == 2
    R3 = 'Melanoma';
